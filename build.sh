...
if    [ ! -z "${REBUILDLIBS}" ] \
   ...
   || [ ! -f "${LIBINSTFULL}/lib/libz.a" ]; then
    ...
    gitCheckout "https://github.com/madler/zlib.git" "v1.2.13" "${LIBBUILD}"
    ...
    if [[ $BUILDWHAT == *"xbuild"* ]]; then
        ...
        xcodebuild -project "${LIBBUILDOUT}/zlib.xcodeproj" \
                   -target zlib_static \
...
if    [ ! -z "${REBUILDLIBS}" ] \
   || [ ! -d "${PKGROOT}/${TARGET}" ]; then
    ...
    mv ${LIBINSTFULL}/lib/libz.a "${PKGROOT}/${TARGET}/${LIBNAME}.a"
